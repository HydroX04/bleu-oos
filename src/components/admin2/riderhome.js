...
import Swal from 'sweetalert2';
  ...
  async function fetchCustomerLocationFromBackend(orderId) {
    ...
    const tries = [
        `https://ordering-service-8e9d.onrender.com/delivery/order/${orderId}/customer/location`,
        `https://ordering-service-8e9d.onrender.com/delivery/order/${orderId}/customer`,
        `https://authservices-npr8.onrender.com/users/orders/${orderId}/customer/location`
    ];
  ...
  const navigate = useNavigate();
    ...
    useEffect(() => {
      if (authToken) {
        setUserLoading(true);
        fetch("https://authservices-npr8.onrender.com/auth/users/me", {
          headers: { "Authorization": `Bearer ${authToken}` }
        })
          .then(res => {
            if (res.status === 401) {
    ...
    useEffect(() => {
      ...
      const updateLocation = async (lat, lng) => {
        try {
          const response = await fetch(`https://ordering-service-8e9d.onrender.com/delivery/rider/${riderId}/location`, {
            method: 'PUT',
            headers: {
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
      try {
        ...
        const cartResponse = await fetch(`https://ordering-service-8e9d.onrender.com/cart/rider/orders/${orderId}/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`
        ...
        const deliveryResponse = await fetch(`https://ordering-service-8e9d.onrender.com/delivery/orders/${orderId}/status`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`
        ...
        Swal.fire({
          ...
          preConfirm: async () => {
            ...
            try {
              ...
              const uploadResponse = await fetch(`https://ordering-service-8e9d.onrender.com/cart/rider/orders/${orderId}/delivery-image`, {
                method: 'POST',
                headers: {
                  'Authorization': `Bearer ${authToken}`,
                },
